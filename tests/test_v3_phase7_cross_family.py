...
def _jvm_manifest(*, suspend: bool = False) -> JvmSourceManifest:
    ...
    return JvmSourceManifest(FEATURE, "3.0.0.dev0", (mode, payload, api))
...
def test_copied_cross_family_codegen_is_typed_recursive_and_hidden(tmp_path: Path):
    ...
    assert "supernote_v3_cross_to_jvm_" in helpers
    assert "supernote_v3_cross_from_jvm_" in helpers
...
def test_composite_suspend_cross_family_route_uses_typed_phase8_completion(
    ...
    assert "supernote_v3_cross_to_jvm_" in generated
    assert "supernote_v3_cross_from_jvm_" in generated
