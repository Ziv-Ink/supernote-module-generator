...
""",
    ...
    assert "supernote_wrap_v3_object_0" in source
...
""",
    ...
    assert "supernote_v3_throw_conversion_failure" in source
...
def test_recursive_copied_free_function_uses_v3_renderer_without_object_leaf(
    tmp_path: Path,
):
...
""",
    ...
    assert "::drawing::Point supernote_v3_from_js_" in source
    assert "facebook::jsi::Value supernote_v3_to_js_" in source
...
""",
    ...
    assert "supernote_v3_object_registry(runtime)" in source
    ...
    js_identity_lookup = source.index(
        "supernote_v3_object_registry(runtime)", scheduled
    )
    ...
    assert "supernote_v3_object_registry" not in worker_capture
