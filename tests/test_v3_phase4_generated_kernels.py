...
from supernote_module_generator.binding_codegen import render_v2_feature_jsi
...
def test_v3_feature_translation_units_include_shared_conversion_kernel(
    tmp_path: Path,
):
    digest = "a" * 64
    feature_id = "supernote:feature:0123456789abcdef"
    cpp = render_v2_feature_jsi(
        tmp_path,
        module_name="Drawing",
    ...
    jvm = render_jvm_feature_jsi(
        JvmSourceManifest(feature_id, "3.0.0.dev0", ()),
        SemanticApi(),
        feature_id=feature_id,
        module_name="Drawing",
        conversion_digest=digest,
    ...
    expected = (
        "// Supernote V3 conversion plan SHA-256: " + digest + "\n"
