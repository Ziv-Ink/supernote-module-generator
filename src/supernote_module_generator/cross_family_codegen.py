"""Generate copied C++ to JVM internal-route conversions for generated values."""
...
def _to_name(value: SemanticType) -> str:
    return f"supernote_module_cross_to_jvm_{_suffix(value)}"


def _from_name(value: SemanticType) -> str:
    return f"supernote_module_cross_from_jvm_{_suffix(value)}"
...
class CrossFamilyRenderer:
    ...
    def _helper_route(self, method: str, descriptor: str) -> str:
        ...
        return _route_expression(
            f"jvm-module-cross-helper:{method}:{descriptor}",
    ...
    def _render_to(self, value: SemanticType) -> str:
        ...
        if kind is SemanticTypeKind.SCALAR:
            ...
            lines.extend([
                ...
                f"  auto route = {_route_expression('jvm-module-cross-enum-from:' + value.type_id, adapter, descriptor, 'fromName')};",
                "  jvalue arguments[1]{};",
            ...
            lines.extend([
                f"  auto route = {_route_expression('jvm-module-cross-value:' + value.type_id, _adapter_class(jvm_value.constructor.adapter_identity), jvm_value.constructor.adapter_descriptor, 'invoke')};",
    ...
    def _render_from(self, value: SemanticType) -> str:
        ...
        if kind is SemanticTypeKind.NULLABLE:
            ...
            if kind is SemanticTypeKind.SCALAR:
                ...
                lines.extend([
                    f"  auto route = {_route_expression('jvm-module-cross-enum-name:' + value.type_id, adapter, enum_descriptor, 'name')};",
                    "  jvalue arguments[1]{}; arguments[0].l = value;",
                ...
                for index, cpp_field in enumerate(cpp_value.fields):
                    ...
                    lines.append(f"  auto field_route_{index} = {_route_expression('jvm-module-cross-field:' + field.field_id, adapter, field.getter_descriptor, 'get')};")
